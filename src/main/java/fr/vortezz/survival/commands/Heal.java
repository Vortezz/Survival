...
public class Heal implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (config.getBoolean("features.commands.heal")) {
				if (player.hasPermission("survival.heal")) {
					if (args.length > 0 && player.hasPermission("survival.heal.others")) {
						Player target = Bukkit.getPlayer(args[0]);
						if (target != null && target != player) {
							target.setHealth(20);
							target.setFoodLevel(20);
							target.setSaturation(1);
							target.sendMessage(messages.getString("healmsg"));
							player.sendMessage(messages.getString("healother") + target.getName() + " !");
							return true;
						}
					}
					player.setHealth(20);
